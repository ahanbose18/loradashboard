...
#endif
extern int16_t iStatus;
extern uint16_t us16TimerCnt;
extern	char appKey[];
extern 	char appEUI[];
...
ATEerror_t Lora_AsyncDownLinkData(sReceivedDataBinary_t *PtrStructData)
{
  ...
  char *ptrChr;
	...
	memset((void*)PtrDataFromNetwork, 0, sizeof(PtrDataFromNetwork));
...
void Lora_fsm(void)
{
  ...
  switch (DeviceState)
  {
    case DEVICE_INIT:
    {
	  ...
	  Lora_SetDataRate(DR_3);		/* Uncomment if required SF8  - BW125 */
      if (LoraModuleRetCode == MODULE_READY)
      {
			keyCharToInt(appEUI, key, 16);
			us16TimerCnt = LoRa_SetAppID(key);

			/* set the APPKEY based on lora node */
			keyCharToInt(appKey, key, 16);
			ret = LoRa_SetKey(AT_APPKEY, key);
...
//	    HW_UART_Modem_Flush();
      switch (LoraCmdRetCode)
      {
...
#endif
        {
          ...
          TimerSetValue(&JoinProtectTimer, 10000);
...
#ifdef USE_I_NUCLEO_LRWAN1
          TimerStop(&JoinProtectTimer);   /* Stop the protect Timer*/
#endif
          DBG_PRINTF("Nwk Joined\n");
        }
        else
...
//		HAL_GPIO_TogglePin(LED2_GPIO_PORT, LED2_PIN);
        ...
        TimerInit(&DemoLedTimer,  Lora_OnLedTimerEvent);
        TimerSetValue(&DemoLedTimer,  250);
        TimerStart(&DemoLedTimer);
		...
		if (LoraCmdRetCode == 0)
		{
			u16Int = atoi(StructDownLinkData.Buffer);
			if (u16Int == 0)
                   ...
                   LoRaDriverParam_t *PtrLoRaDriverParam)
{
  ...
  /***************************************************************************/
#if 0
  /* since we fixe the FWversion to USI_FW_V30 this statement is not necessary*/
  /* could be enable for user who wants to discriminate the FW version*/
#if defined (USE_I_NUCLEO_LRWAN1) /*&& defined (MODEM_IN_SLEEP_MODE)*/
  uint8_t   RetCode;
