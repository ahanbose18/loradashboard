...
/***************/
  ...
  /* EXTI interrupt init*/
  HAL_NVIC_SetPriority(EXTI4_15_IRQn, 1, 0);	//ksa
...
uint16_t HW_GetBatteryLevel( void ) 
{
  ...
  measuredLevel = HW_AdcReadChannel(ADC_CHANNEL_VREFINT);
	return measuredLevel;		/* added by ksa 21-06-21  raw data from force sensor */
...
void HW_AdcInit(void)
{
  if (AdcInitialized == false)
  {
	...
	/*** added to include analog input, KSA */
    GPIO_InitTypeDef initStruct = {0};
    initStruct.Mode = GPIO_MODE_ANALOG;
    initStruct.Pull = GPIO_NOPULL;
    initStruct.Speed = GPIO_SPEED_HIGH;

    HW_GPIO_Init(FSR_PORT, FSR_PIN, &initStruct);
	/* ksa end */
